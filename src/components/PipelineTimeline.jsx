...
function getStage(nowSecs) {
  ...
  if (h < 2)  return { nodeIdx: 1, phase: 'Agents Running Now' };
  if (h < 4)  return { nodeIdx: 2, phase: 'Briefs Generating Now' };
  if (h < 6)  return { nodeIdx: 3, phase: 'Deals Delivering Now' };
  return        { nodeIdx: 0, phase: 'Dead Zone — Accepting Submissions' };
}
...
const S = {
  // Inline phase pill at top-left of the slim band
  phaseRow:  { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 12 },
  phaseLeft: { display: 'flex', alignItems: 'center', gap: 8 },
  ...
  // Slim animated track
  trackRow:  { position: 'relative', height: 64, marginLeft: 60, marginRight: 60 },
...
export function PipelineTimeline() {
  ...
  useEffect(() => {
    function tick() {
      ...
      const { nodeIdx, phase } = getStage(nowSecs);
  ...
  return (
    <div className="pipeline-timeline pipeline-timeline-slim">
