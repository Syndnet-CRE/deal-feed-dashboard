import { BUY_BOXES } from '../data/mockData';
import { I } from '../components/Icons';

export function BuyBoxesView({ onCreate }) {
  const failed = BUY_BOXES.filter(b => b.status === "Coverage Failed");
  return (
    <div className="page">
      <div className="page-head">
        <div>
          <h1 className="page-title">Buy Boxes</h1>
          <div className="page-sub">Manage the criteria that drive each nightly deal-feed run · {BUY_BOXES.filter(b => b.status === "Active").length} active</div>
        </div>
      ...
      {failed.length > 0 && (
        <div className="callout">
          ...
          <div className="cal-text">
            <b>Coverage Failed:</b> Charlotte — IOS could not be activated. We do not yet have parcel data for Mecklenburg County. <a href="#" style={{ color: "#FF7378", fontWeight: 700, textDecoration: "underline" }}>Edit geography</a> or contact support.
          </div>
        </div>
      )}
